...
#[must_use] pub fn decompress(data: &[u8], len: u32, adler32: u32) -> Option<Vec<u8>> {
    ...
    assert!(sz <= len, "LZSS decompresser wrote beyond allocated buffer"); //program is no longer stable, crash (this should never happen)
    unsafe { decmpvec.set_len(cast_force!(sz, usize)); }
    if unsafe { local_adler32(decmpvec.as_ptr(), cast_force!(len, i32)) } == adler32 { 
        Some(decmpvec)
    } else {
        None
    }
...
#[must_use] pub fn compress(data: &[u8]) -> Vec<u8> {
    ...
    assert!(sz <= data.len(), "LZSS compresser wrote beyond allocated buffer"); //program is no longer stable, crash (this should never happen)
