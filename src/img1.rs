...
pub fn create(buf: &[u8], args: &mut Args) {
    ...
    let encd = &encrypt(cipher, KEY_837, None, res)
        .unwrap_or_else(|e| panic!("{} to encrypt img2 sha1, error: {e}", "Failed".red()))[0..0x10];
...
pub fn parse(file: &[u8], args: &mut Args) {
    ...
    let is_ios = head.platform == S5L8900_HEADER_MAGIC
        || (head.platform == S5L8720_HEADER_MAGIC && head.version == IMG1_FORMAT_1);

    let cipher = Cipher::aes_128_cbc();
    let datstart = if head.platform == S5L8900_HEADER_MAGIC
        || head.platform == S5L8702_HEADER_MAGIC
        || head.platform == S5L8442_HEADER_MAGIC
    {
        0x800
    } else if head.platform == S5L8720_HEADER_MAGIC || head.platform == S5L8730_HEADER_MAGIC {
        0x600
    } else if head.platform == S5L8723_HEADER_MAGIC
        || head.platform == S5L8740_HEADER_MAGIC
        || head.platform == S5L8443_HEADER_MAGIC
    {
        0x400
    } else {
        0x800 //idk
    ...
    let sig_off = datstart
        + if head.footer_sig_off < lenalign + 0x300 {
            // 8900 case
    ...
    } else if head.platform == S5L8900_HEADER_MAGIC
        || (head.platform == S5L8720_HEADER_MAGIC && head.version == IMG1_FORMAT_1)
    {
    ...
    if args.verify {
        ...
        if check.is_ok() && file.len() >= i + head.footer_cert_len as usize {
            let leafcert = verify_cert(
                &file[range_size(i, head.footer_cert_len as usize)],
                &mut is_valid,
            );
