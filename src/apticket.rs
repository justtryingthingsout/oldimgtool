...
use {
    crate::utils::{phf_map, verify_cert, DeviceInfo},
    asn1_rs::{oid, Any, FromDer, OctetString, Oid, Sequence, SerializeError, Set, Tag, ToDer},
    colored::Colorize,
    openssl::{hash::MessageDigest, rsa::Padding, sign::Verifier},
    phf::Map,
    std::str,
};
...
#[must_use]
pub fn partial_sha1(buf: &[u8]) -> Option<Vec<u8>> {
    #[expect(nonstandard_style, dead_code)]
    mod bindings {
        include!(concat!(env!("OUT_DIR"), "/bindings.rs"));
    }
    use bindings::{shaSuccess, SHA1HashSize, SHA1Input, SHA1Reset, SHA1ResultPartial};
...
pub fn parse(
    ...
    let (_, nonce) = Sequence::from_der_and_then(apticket, |d| {
        ...
        Set::from_der_and_then(d, |d| {
            ...
            while let Ok((i, cur)) = Any::from_der(d) {
                // Parse context-specific like this
                match cur.tag() {
                    Tag(1) => {
                        // ECID
                    ...
                    Tag(3) => {
                        // BORD
                        dfo.bdid =
                            Some(vec![u32::from_le_bytes(cur.as_bytes().try_into().unwrap())]);
                    }
                    Tag(4) => {
                        // PROD
