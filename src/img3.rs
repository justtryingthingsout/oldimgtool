...
use {
    crate::{
        apticket,
        utils::{
            checkvalid_decry, compress, create_complzss_header, do_resize, override_types,
            print_unknown_val, range_size, revstr_from_le_bytes, verify_cert, write_out,
            write_outpath, BinWrite, Cursor, DeviceInfo, IMG3ObjHeader, IMG3TagHeader,
            IMG3TagString, IMG3KBAG, IMG3_GAT_CERTIFICATE_CHAIN, IMG3_GAT_DATA, IMG3_GAT_KEYBAG,
            IMG3_GAT_RANDOM, IMG3_GAT_SIGNED_HASH, IMG3_GAT_TYPE, IMG3_GAT_UNIQUE_ID,
            IMG3_GAT_VERSION, IMG3_HEADER_CIGAM, IMG3_TAG_BAT0, IMG3_TAG_BAT1, IMG3_TAG_BATF,
            IMG3_TAG_BOARD_TYPE, IMG3_TAG_CERT, IMG3_TAG_CERTIFICATE_CHAIN, IMG3_TAG_CHG0,
            IMG3_TAG_CHG1, IMG3_TAG_CHIP_TYPE, IMG3_TAG_DATA, IMG3_TAG_DIAG, IMG3_TAG_DTRE,
            IMG3_TAG_GLYC, IMG3_TAG_GLYP, IMG3_TAG_HARDWARE_EPOCH, IMG3_TAG_IBEC, IMG3_TAG_IBOT,
            IMG3_TAG_IBSS, IMG3_TAG_ILLB, IMG3_TAG_KEYBAG, IMG3_TAG_KRNL, IMG3_TAG_LOGO,
            IMG3_TAG_NONCE, IMG3_TAG_OVERRIDE, IMG3_TAG_PRODUCTION_STATUS, IMG3_TAG_RANDOM,
            IMG3_TAG_RANDOM_PAD, IMG3_TAG_RDSK, IMG3_TAG_RDTR, IMG3_TAG_RECM, IMG3_TAG_RKRN,
            IMG3_TAG_RLGO, IMG3_TAG_SCAB, IMG3_TAG_SECURITY_DOMAIN, IMG3_TAG_SECURITY_EPOCH,
            IMG3_TAG_SIGNED_HASH, IMG3_TAG_TYPE, IMG3_TAG_UNIQUE_ID, IMG3_TAG_VERSION,
            PLAUSABLE_PKCS1,
        },
        Args,
    },
    asn1_rs::{FromDer, OctetString, Sequence},
    binrw::BinReaderExt,
    colored::Colorize,
    openssl::{
        hash::MessageDigest,
    ...
    plist::Value,
    std::{borrow::Cow, io::Read, result::Result},
};
...
fn parse_ext(
    ...
    unsafe {
        ...
        use std::{
            ffi::{CStr, CString},
            ptr::{addr_of_mut, null_mut}, // null,
            slice::from_raw_parts,
        };

        let leafder = leafcert.to_der().unwrap();
        let mut x509_loc_ptr = leafder.as_ptr();
        let unsafe_x509_cert = d2i_X509(
            null_mut(),
            addr_of_mut!(x509_loc_ptr),
            leafder.len().try_into().unwrap(),
        );
        ...
        if as_obj.is_null() {
            ...
            panic!(
                "Failed to create NID with error: {}",
        ...
        eprintln!("Found Apple certificate signing extension, parsing it");
        ...
        assert_eq!(
            img3_slice[0..4],
            IMG3_HEADER_CIGAM,
            "Apple certificate signing extension does not contain IMG3 data"
        );
        args.verify = false;
...
fn cert_tag(
    ...
    if args.verify {
            ...
            if let Some(devinfo) = devinfo {
                if has_ext {
                    if devinfo.sdom.is_none() || devinfo.prod.is_none() || devinfo.cpid.is_none() {
                        eprintln!(
                            "Required constraints do not exist in the certificate extension, \
                             marking this image invalid."
                        );
                        *is_valid = false;
                    }
                } else {
                    eprintln!("No extension found, assuming old iBoot component");
                }
            }
            eprintln!(
                "This image is {}{}",
                ...
                if *is_valid {
                    // no point showing the info if it's invalid anyways
                    devinfo.as_mut().map_or_else(
        ...
        eprintln!("Found a CERT tag with invalid certificates, skipping verification");
        return true;
    };
    false
}
...
fn data_tag(
    ...
    taghead: &mut IMG3TagHeader,
    file: &mut Vec<u8>,
    i: usize,
) {
    if let Some(datapath) = args.setdata.as_mut() {
        let mut datafile = read_all!(datapath).unwrap();
        if args.comp {
    ...
    if let (Some(argkey), Some(path)) = (args.key.as_deref(), args.outfile.as_mut()) {
        ...
        let mut decrypter =
            Crypter::new(cipher, Mode::Decrypt, &key_bytes, Some(&iv_bytes)).unwrap();
        decrypter.pad(false);
        let mut buf = vec![0; taghead.buf.len() + cipher.block_size()];
        decrypter.update(&taghead.buf, &mut buf).unwrap();
        let mut flag = true;
        decrypter.finalize(&mut buf).unwrap_or_else(|e| {
            use {openssl::error::Error, std::io::Write};
            let errstack = e.errors();
            let cond = errstack.len() > 1;
            eprint!(
                "Got {} whilst finalizing the decryption: ",
                if cond { "multiple errors" } else { "an error" }
            );
            if cond {
                for err in &errstack[..(errstack.len() - 1)] {
                    eprint!("{}, ", err.reason().unwrap_or("No reason given"));
                }
            }
            eprintln!(
                "{}",
            ...
            eprint!("This can sometimes still contain valid data, continue? [y/N]: ");
        ...
        if flag {
            ...
            if args.dec {
                ...
                write_outpath(
                    path,
                    &checkvalid_decry(&buf, head.img3_type, args.ext).unwrap_or(buf),
                );
            }
        }
            ...
            && args.shshpath.is_none()
        {
            write_outpath(
                path,
                &checkvalid_decry(&taghead.buf, head.img3_type, args.ext)
                    .unwrap_or_else(|| taghead.buf.clone()),
            );
        }
    }
}
...
fn kbag_tag(
    ...
    if args.all || args.keybags {
        ...
        eprintln!("\tKey size: AES{}", ksize * 8);
        eprintln!("\tIV: {iv}", iv = hex::encode(keyhead.iv_bytes));
        eprintln!(
            "\tKey: {key}",
            key = hex::encode(&keyhead.key_bytes[..ksize])
        );
        ...
        if keyhead.selector == sel {
            ...
            if !args.onlykbag {
                ...
                let mut encrypter =
                    Crypter::new(cipher, Mode::Encrypt, &key_bytes, Some(&iv_bytes)).unwrap();
                encrypter.pad(false);
                let mut buf = vec![0; padded.len() + cipher.block_size()];
                let count = encrypter.update(&padded, &mut buf).unwrap();
...
fn new_kbag(sects: &mut Vec<IMG3TagHeader>, args: &Args) {
    if let Some(kbg) = &args.setkbag {
        ...
        if !args.onlykbag {
            ...
            let buf = encrypt(cipher, &key_bytes, Some(&iv_bytes), &padded).unwrap_or_else(|e| {
                panic!(
                    "{} to encrypt img3 DATA section, error: {e}",
                    "Failed".red()
                )
            });
...
pub fn create(mut buf: Vec<u8>, args: &mut Args) {
    ...
    if let Some(sigpath) = args.sigpath.as_mut() {
        let sig = read_all!(sigpath).unwrap();
        let siglen = sig.len();
        sects.push(IMG3TagHeader {
    ...
    if let Some(certpath) = args.certpath.as_mut() {
        let cert = read_all!(certpath).unwrap();
    ...
    let signed: u32 = sects
        .iter()
        .filter_map(|x| {
            (![IMG3_GAT_SIGNED_HASH, IMG3_GAT_CERTIFICATE_CHAIN].contains(&x.tag))
                .then_some(x.skip_dist)
        })
        .sum();
...
fn parse_blob(
    ...
    if let Some(shshpath) = args.shshpath.as_mut() {
        if !is_ext {
            ...
            let buf = read_all!(shshpath).expect("Failed to read blob");
            ...
            if buf[..2] == [0x30, 0x82] {
                ...
                *apticket = Some(buf);
                ...
                if buf == *b"<?xml" {
                    // SHSH blob
                    let fullblob =
                        Value::from_reader(Cursor::new(buf)).expect("Failed to read blob");
                    *apticket = fullblob
                        .as_dictionary()
                        .and_then(|dict| dict.get("APTicket")?.as_data())
                        .map(<[u8]>::to_vec);

                    if let Some(imgblob) = fullblob
                        .as_dictionary()
                        .and_then(|dict| dict.get(imgtype)?.as_dictionary())
                    {
                        part_dgst = imgblob
                            ...
                            .expect(
                                "Did not find the required PartialDigest (personalization not \
                                 required?)",
                            )
                            .to_vec();
                    ...
                    head.buf_len = oldsign + cast_force!(blob.len(), u32);
                    head.skip_dist =
                        oldsign + cast_force!(blob.len(), u32) + /* sizeof IMG3ObjHeader */ 0x14;
                    file.splice(
                        /* sizeof IMG3ObjHeader */ 0x14 + oldsign as usize..,
                        blob,
                    );
...
/// # Panics
/// Panics if the buffer does not contain a valid IMG3 file, or the arguments
/// are incorrect.
...
pub fn parse(
    ...
    let ecid_size = if !is_ext
        && (0x14 + head.signed_len as usize)
            .checked_sub(0x40)
            .is_some()
        && (file[range_size(0x14 + head.signed_len as usize - 0x40, 4)] == IMG3_GAT_UNIQUE_ID
            || file[range_size(0x14 + head.signed_len as usize - 0x40, 4)] == IMG3_GAT_RANDOM)
    // Haywire specific (?)
    {
        0x40
    } else {
    ...
    let sha1 = if head.signed_len != 0 {
        sha1(&file[range_size(0xC, 0x8 + head.signed_len as usize)])
    } else {
        [0; 20]
    };
    let partialsha1 = if head.signed_len != 0 {
        crate::apticket::partial_sha1(&file[range_size(0xC, 0x8 + head.signed_len as usize)])
            .unwrap()
    } else {
        Vec::new()
    };
    ...
    while i < 20 + head.buf_len as usize {
        //tag parse loop until EOF
        ...
        match tag.as_str() {
            IMG3_TAG_VERSION => {
                ...
                if args.all || args.ver {
                    eprintln!(
                        "{}Version string: {vers}",
                        if args.all { "\t" } else { "" },
                        vers = vershead.str_bytes
                    );
                } else if let Some(vers) = &args.setver {
                    eprintln!("Version was: {oldv}", oldv = vershead.str_bytes);
                    ...
                    eprintln!("Version is: {vers}", vers = vershead.str_bytes);
                }
            }
            IMG3_TAG_TYPE => {
            ...
            IMG3_TAG_KEYBAG => {
                if kbag_tag(args, &mut head, &mut taghead, &mut file, i, data) {
                    continue;
                }
            }
            IMG3_TAG_CERTIFICATE_CHAIN => {
                ...
                if let Some(ref apticket) = apticket {
                    let is_ok = if cached_res {
                        cached_res
                    } else {
                        apticket::validate(apticket)
                    };
                    if is_ok {
                        let (_, nonce) = apticket::parse(
                            apticket,
                            head.img3_type,
                            &sha1,
                            &partialsha1,
                        ...
                        if head.img3_type != IMG3_TAG_IBSS && head.img3_type != IMG3_TAG_ILLB {
                            // only these 2 don't require nonce
                ...
                if let Some(path) = args.savecertpath.as_mut() {
                    write_out(path, &taghead.buf);
                } else if let Some(certpath) = args.certpath.as_mut() {
                    let certfile = read_all!(certpath).unwrap();
                    let taglen = 12 + cast_force!(taghead.buf.len() + taghead.pad.len(), u32);
                    do_resize(&mut head, &mut taghead, &mut file, i, taglen, certfile);
                }
                if cert_tag(args, &head, &taghead, &file, &shshdata, is_valid, devinfo) {
                    break;
                }
            }
            IMG3_TAG_UNIQUE_ID => {
                //number, but this has a uint64 size
            ...
            IMG3_TAG_SECURITY_DOMAIN => {
                ...
                if is_ext {
                    // SDOM can only be checked from certificate extension
                    if let Some(ref mut devinfo) = devinfo {
            ...
            IMG3_TAG_SIGNED_HASH => {
                if let Some(path) = args.savesigpath.as_mut() {
                    write_out(path, &taghead.buf);
                } else if let Some(sigpath) = args.sigpath.as_mut() {
                    let hashfile = read_all!(sigpath).unwrap();
                    let taglen = 12 + cast_force!(taghead.buf.len() + taghead.pad.len(), u32);
            ...
            | IMG3_TAG_HARDWARE_EPOCH) => {
                ...
                match x {
                    ...
                    _ => {
                        unreachable!("This match should be unreachable")
                    }
                }
            }
    ...
    if !sawcert && args.verify {
        ...
        if let Some(ref apticket) = apticket {
            ...
            if is_ok {
                let (mut devinfo, nonce) = apticket::parse(
                    apticket,
                    head.img3_type,
                    &sha1,
                    &partialsha1,
            ...
            eprintln!(
                "File cannot be verified without either a CERT and SHSH tag, or an APTicket."
            );
        }
    }
